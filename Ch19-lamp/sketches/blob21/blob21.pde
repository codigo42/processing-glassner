...
void buildImage() {
   color cool = color(18, 69, 94);
   color hot = color(207, 152, 14);
   float midT = 0.4;
   for (int y=0; y<height; y++) {
      for (int x=0; x<width; x++) {
         if (Temperature[y][x] > midT) Img.pixels[(y*width)+x] = hot;
                                  else Img.pixels[(y*width)+x] = cool;
      }
   }
}
   ...
   void render() {
      ...
      for (int y=loy; y<hiy; y++) {
         for (int x=lox; x<hix; x++) { 
            ...
            float v = exp(-4*h*h);
            v = map(v, 1, 0.01831564, 0, 1);  // map (1,exp(-4)) to (0,1)
            Temperature[y][x] += v;
         }
      }
   }
