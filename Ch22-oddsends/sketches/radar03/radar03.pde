...
class Blip {
   ...
   Blip() {
      float smallSize = Window/80.0;
      float bigSize = 2.25*smallSize;
      angle = random(0, 360);
      radius = random(RingLo*Window/2.0, (RingHi*Window/2.0)-bigSize);
      size = random(smallSize, bigSize);
   }

   void render(float pulseAngle) {
      float glowLength = PulseTrailWidth*1.5;
      float angleDiff = pulseAngle - angle;
      if (angleDiff < 0) angleDiff += 360;
      float opacity = lerp(255, 0, angleDiff/glowLength);
      if ((opacity < 0) || (opacity > 255)) return;
      noFill();
      strokeWeight(2);
      stroke(red(BrightGreen), green(BrightGreen), blue(BrightGreen), opacity);
      ellipse(Cx+radius*cos(radians(angle)), Cy+radius*sin(radians(angle)), 2*size, 2*size);
      noStroke();
      fill(red(BrightGreen), green(BrightGreen), blue(BrightGreen), opacity);
      ellipse(Cx+radius*cos(radians(angle)), Cy+radius*sin(radians(angle)), 1.5*size, 1.5*size);
   }	
}
...
void draw() {
   ...
   for (int b=0; b<NumBlips; b++) {
      BlipList[b].render(angle);
   }
}

void makeBlips() {
   BlipList = new Blip[NumBlips];
   for (int b=0; b<NumBlips; b++) {
      BlipList[b] = new Blip();
   }
}
