...
void draw() {
   ...
   float lightDirX = lerp(-1, 1, noise((frameCount+151)*.02));
   float lightDirY = lerp(-1, 1, noise((frameCount+307)*.02));
   directionalLight(128, 128, 100, lightDirX, lightDirY, -.2);

   float lightDirX2 = lerp(-1, 1, noise((frameCount+500)*.02));
   float lightDirY2 = lerp(-1, 1, noise((frameCount+800)*.02));
